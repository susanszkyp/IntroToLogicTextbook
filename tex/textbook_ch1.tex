...
\fpbox{It is important to note that in a predicate like $\pred{4}{9}$, the number $4$ is called its \textit{arity}, and the number $9$ is called its \textit{index}. For example, though $4$ may look like the \textit{power} or \textit{exponent}, this is just because of the similar notation. In reality, these two ideas are not related in any way, and should not be confused.}
...
\fpbox{It is important that you do not mix the notation freely! In logic, different ways of writing the same thing may have a different meaning. For example, $p$, $P$, $\mathbf{P}$ and $\mathfrak{P}$ will all mean different things. You should be careful which is used at any one point (see immediately below for a distinction!).}
